...
// Create contacts table & indexes if database is available
pool.query(`CREATE TABLE IF NOT EXISTS contacts (
    ...
    if (err) {
        console.error('Error creating table or connecting to PostgreSQL:', err.message);
    } else {
        console.log('Connected to PostgreSQL and verified contacts table.');
        ...
        pool.query("CREATE INDEX IF NOT EXISTS idx_contacts_created_at ON contacts(created_at DESC)", () => { });
        pool.query("CREATE INDEX IF NOT EXISTS idx_contacts_is_contacted ON contacts(is_contacted)", () => { });
    }
});
...
app.post('/api/contact', async (req, res) => {
    ...
    try {
        ...
        await sendContactEmails({ name: cleanName, email: cleanEmail, phone: cleanPhone, subject, message: cleanMessage });
